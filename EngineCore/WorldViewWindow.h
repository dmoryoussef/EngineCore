...
protected:
	...
	void constructComponent(BaseNode* pBaseNode, OutputBuffer *pBuffer)
	{
		...
		ecsRenderer.render(pBaseNode, &renderer, Vector2(0, 0), Vector2(m_nWidth, m_nHeight));
		

		pBuffer->set("Mouse: " + vCurrentMousePosition.toString(), m_nWidth - 20, 2, FG_WHITE);
		pBuffer->set("World: " + WorldPosition(vCurrentMousePosition, Vector2(0, 0), Position, m_fScreenScale).toString(), m_nWidth - 20, 3, FG_WHITE);
