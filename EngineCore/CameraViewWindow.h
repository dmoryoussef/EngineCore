...
private:
	...
	void constructComponent(BaseNode* pBaseNode, OutputBuffer *pBuffer)
	{
		...
		pBuffer->set(" Screen: " + vCurrentMousePosition.toString(), m_nWidth - 25, 2, FG_WHITE);
		pBuffer->set("  World: " + WorldPosition(vCurrentMousePosition, vCurrentCameraPosition.toVec2(), Position, vCurrentCameraPosition.Z).toString(), m_nWidth - 25, 3, FG_WHITE);
		pBuffer->set("   Zoom: " + thingToString<float>(vCurrentCameraPosition.Z), m_nWidth - 25, 4, FG_WHITE);
		pBuffer->set("    FPS: " + thingToString<float>((1 / m_fDeltaTime ) * 1000.0), m_nWidth - 25, 5, FG_WHITE);
		pBuffer->set("Runtime: " + thingToString<float>(m_fTotalTime), m_nWidth - 25, 6, FG_WHITE);
	}
